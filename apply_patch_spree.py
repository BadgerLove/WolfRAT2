...
# 1. Update minimum window size
content = content.replace("self.setMinimumSize(1100, 750)", "self.setMinimumSize(960, 650)")
content = content.replace("self.resize(1300, 850)", "self.resize(1000, 720)")

# 2. Extract Spree code from MessagesTab
...
# 3. Add to MainWindow
if "self.spree_tab = SpreeTab" not in content:
    ...
    content = content.replace(
        "self.signals.players_signal.connect(self.messages_tab.check_sprees)",
...
# Remove spree UI from MessagesTab
spree_ui_start = "        # --- Killing Spree Announcer ---"
spree_ui_end = "        layout.addWidget(spree_group)\n"
content = remove_between(content, spree_ui_start, spree_ui_end)

# "Hide" message log from layout instead of completely deleting the QTextEdit object
# Original:
#         log_group.setLayout(log_layout)
#         layout.addWidget(log_group)
hide_log = "        log_group.setLayout(log_layout)\n        # layout.addWidget(log_group) # Removed to save space"
content = content.replace("        log_group.setLayout(log_layout)\n        layout.addWidget(log_group)", hide_log)

# Also remove the check_sprees method entirely from MessagesTab (SpreeTab now has it)
...
content = remove_method(content, "    def check_sprees(self, players):", "class MissionsStore:")

with open(FILE_PATH, 'w', encoding='utf-8') as f:
    f.write(content)

print("Patch 1 (Messages/Spree) applied.")
