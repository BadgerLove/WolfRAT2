...
class WolfProtocol:
        ...
        try:
            while self._running and self.connected and self.sock:
                try:
                    ...
                    wire_log(f"  hex: {data[:2000].hex()}")
                    wire_log(f"  ascii: {data[:2000]}")
    ...
    def _process_buffer(self):
        ...
        while len(self._buffer) >= self.HEADER_SIZE:
            ...
            wire_log(f"  payload hex: {payload_bytes[:2000].hex()}")
            wire_log(f"  payload ascii: {payload_bytes[:2000]}")
...
class ServerManager:
    def __init__(self):
        ...
        self._on_chat = None
        self._on_raw_chat = None  # raw chat payload before overlap detection
        ...
        self._expect_chat_response = False  # True after 'chat get' sent, so response is tagged
    ...
    def send(self, command, quiet=False):
        if not quiet:
            ...
            # Inject a quiet tag so the UI ConsoleTab can filter it if desired
            self._log(f"__QUIET__>> {command}")
        ...
        while self._polling and self.proto.connected:
            try:
                ...
                if self._polling and self.proto.connected:
                    ...
                    if silence > 60:
                        wire_log(f"CONNECTION LOST: no response in {silence:.0f}s")
                        self._log("Connection timed out (no data for 60s)")
                        self.proto._do_disconnect("Connection timed out")
                        break
                    elif silence > 30:
                        if not stall_logged:
                    ...
                    self._expect_chat_response = True
                    time.sleep(0.3)
    ...
    def _handle_packet(self, payload):
        ...
        # Give a small hint about the payload content for the UI to filter
        hint_prefix = ""
        lower = payload.lower()
        # If we just sent 'chat get', tag the response as chat regardless of content
        # (server announcements like vote messages don't match the Name: message pattern)
        if self._expect_chat_response:
            hint_prefix = "<< CHAT =\n"
            self._expect_chat_response = False
        elif 'autobalance' in lower or 'votepercent' in lower or 'killlimit' in lower:
            hint_prefix = "<< GAME SETTINGS =\n"
        elif '\t' in payload and '#' in payload and 'name' in lower and 'team' in lower:
            hint_prefix = "<< PLAYER LIST =\n"
        elif 'current state' in lower or 'state =' in lower:
            hint_prefix = "<< CURRENT STATE =\n"
        elif '<current mission>' in lower:
            hint_prefix = "<< MISSION LIST =\n"
        elif '\t' not in payload and ':' in payload:
            has_chat = False
        ...
        self._log(f"{hint_prefix}<< {payload[:500]}")
        ...
        #    OR numbered map lines like '0: DM-MapName.bms'
        #    MUST be before chat — mission lines like '0: DM-COD4Killhouse.npj' have colons
        if '<current mission>' in lower:
            wire_log(f'CONTENT MATCH: missions')
            self._parse_missions(payload)
            return
        # Fallback: match numbered map lines (e.g. '0: DM-COD4Shipment.npj - () ()')
        import re as _re
        if _re.search(r'^\d+:\s+\S+\.(bms|npaj|npj)', payload, _re.MULTILINE | _re.IGNORECASE):
            wire_log(f'CONTENT MATCH: missions (numbered format fallback)')
            self._parse_missions(payload)
            return
        ...
        wire_log(f'UNMATCHED: payload[:80]={payload[:80]!r}')
    ...
    def _parse_missions(self, data):
        ...
        # Filter out lines that don't look like mission entries
        import re as _re_m
        missions = [m for m in missions if _re_m.search(r'\.(bms|npaj|npj)', m, _re_m.IGNORECASE)]
        if not missions:
            wire_log(f'MISSIONS: no valid map lines found, keeping existing list')
            return
        # If no <CURRENT MISSION> tag found, mark the first map as current
        has_current = any('<CURRENT MISSION>' in m for m in missions)
        if not has_current and missions:
            missions[0] = missions[0] + ' <> <CURRENT MISSION>'
    ...
    def _parse_chat(self, data):
        # Fire raw chat callback BEFORE overlap detection
        if self._on_raw_chat:
            try:
                self._on_raw_chat(data)
            except Exception:
                pass
        
        new_lines_incoming = [line.strip().rstrip('\r') for line in data.split('\n') if line.strip()]
        
        if not hasattr(self, '_last_raw_chat_lines'):
            self._last_raw_chat_lines = []
            self._chat_msg_id_counter = 0
            
        overlap_size = 0
        max_possible = min(len(self._last_raw_chat_lines), len(new_lines_incoming))
        
        for size in range(max_possible, 0, -1):
            if self._last_raw_chat_lines[-size:] == new_lines_incoming[:size]:
                overlap_size = size
                break
                
        new_lines = new_lines_incoming[overlap_size:]
        self._last_raw_chat_lines = new_lines_incoming
        
        for line in new_lines:
            self._chat_msg_id_counter += 1
            self.chat_messages.append({
                'id': self._chat_msg_id_counter,
